 ...
 * Revision: 3405 
 * 
 * Modified 2011-07-07 by Adam Vessey to change semantics for playlist progression
 *  Justification:  Selecting playlist element(s) performed can be performed by class,
 *      but clips depended upon being "next" to each other (as returned from DOM?).
 *      Now, iterate through the found elements.
 *  Note:   Not sure, but might break dynamic adding/removing of clips.
 */ 
(function($) {
...
//}}}  
		...
		if (manual && opts.loop) {
			self.onBeforeFinish(function(clip) {
                                ...
                                if(!el.hasClass(opts.progressClass))
                                {
                                    if (els.length > idx + 1)
                                    {
                                        els.get(idx + 1).click();
                                    ...
                                    {
                                        //FIXME:  Perhaps there should be another parameter?
                                        //  I don't actually want it to loop, but to play through
                                        //  the songs.  My understanding is that "loop" controls
                                        //  both sequential play and looping...
                                        //  If loop were "false," we wouldn't get to this
                                        //  block of code!
                                        //els.get(0).click();
                                        self.stop();
                                        clearCSS();
